...
class ListPropertiesComponent extends Component {
  ...
  filterByCity = () => {
    this.setState({
      filteredProperties:
        this.state.city !== "All"
          ? this.state.filteredByFor.filter(
              (prop) => prop.city === this.state.city
            )
          : this.state.filteredByFor,
      filteredByCity:
        this.state.city !== "All"
          ? this.state.filteredByFor.filter(
              (prop) => prop.city === this.state.city
  ...
  filterByFor = () => {
    this.setState({
      filteredProperties:
        this.state.for !== "All"
          ? this.state.property.filter((prop) => prop.for === this.state.for)
          : this.state.property,
      filteredByFor:
        this.state.for !== "All"
          ? this.state.property.filter((prop) => prop.for === this.state.for)
  ...
  filterByText = () => {
    this.setState({
      filteredProperties:
        this.state.text !== ""
          ? this.state.filteredByCity.filter((prop) =>
              prop.propertytitle
                .toLowerCase()
                .includes(this.state.text.toLowerCase())
            )
          : this.state.filteredByCity,
    });
  };
  render() {
    return (
